...
var unicodeData = _interopRequireWildcard(_asciiIdentifierData);
...
function Lexer(source) {
  ...
  for (var i = 0; i < stateBinding.option.indent; i += 1) {
    stateBinding.tab += " ";
  }
}
...
Lexer.prototype = {
  ...
  getLines: function getLines() {
    this._lines = stateBinding.lines;
    return this._lines;
  },

  setLines: function setLines(val) {
    this._lines = val;
    stateBinding.lines = this._lines;
  },
  ...
  scanIdentifier: function scanIdentifier(checks) {
    ...
    function isNonAsciiIdentifierStart(code) {
      return _nonAsciiIdentifierStart.arr.indexOf(code) > -1;
    }

    function isNonAsciiIdentifierPart(code) {
      return isNonAsciiIdentifierStart(code) || _nonAsciiIdentifierPartOnly.arr.indexOf(code) > -1;
    }
    ...
    if (!stateBinding.inES6(true)) {
      es5IdentifierNames = _es5IdentifierNames.es5identifiernamesjs;
    ...
    switch (char) {
      ...
      case "0":
        ...
        this.triggerAsync("warning", {
          ...
          return n >= 0 && n <= 7 && stateBinding.isStrict();
        });
        break;
      case "1":
      case "2":
  ...
  scanStringLiteral: function scanStringLiteral(checks) {
    ...
    this.triggerAsync("warning", {
      ...
      return stateBinding.jsonMode && quote !== "\"";
    });
      ...
      if (this.peek() === "") {
        ...
        if (!allowNewLine) {
          ...
          this.triggerAsync("warning", {
            ...
            return !stateBinding.option.multistr;
          });
          ...
          this.triggerAsync("warning", {
            ...
            return stateBinding.jsonMode && stateBinding.option.multistr;
          });
        }
  ...
  scanRegExp: function scanRegExp(checks) {
    ...
    while (index < length) {
      ...
      if (char === "y") {
        ...
        }(groupReferences, groupCount, escapedChars, reg.reg);
    ...
    if (allFlags.indexOf("u") === -1) {
      this.triggerAsync("warning", {
        ...
        return stateBinding.option.regexpu;
      });
    }
  ...
  scanNonBreakingSpaces: function scanNonBreakingSpaces() {
    return stateBinding.option.nonbsp ? this.input.search(/(\u00A0)/) : -1;
  },
  ...
  nextLine: function nextLine(checks) {
    ...
    this.input = this.input.replace(/\t/g, stateBinding.tab);

    // If there is a limit on line length, warn when lines get too
    // long.

    if (!this.ignoringLinterErrors && stateBinding.option.maxlen && stateBinding.option.maxlen < this.input.length) {
  ...
  token: function token() {
    ...
    var create = function (type, value, isProperty, token) {
      ...
      if (type === "(punctuator)") {
        ...
        obj = Object.create(stateBinding.syntax[value] || stateBinding.syntax["(error)"]);
      }
      ...
      if (type === "(identifier)") {
        ...
        if (_lodash2.default.has(stateBinding.syntax, value)) {
          obj = Object.create(stateBinding.syntax[value] || stateBinding.syntax["(error)"]);
        }
      }
      ...
      if (!obj) {
        obj = Object.create(stateBinding.syntax[type]);
      }
      ...
      switch (token.type) {
        ...
        case Token.Identifier:
          this.triggerAsync("Identifier", {
            ...
            name: token.value,
            raw_name: token.text,
            isProperty: stateBinding.tokens.curr.id === "."
          }, checks, function () {
            return true;
          });
        ...
        /* falls through */
        case Token.Keyword:
          return create("(identifier)", token.value, stateBinding.tokens.curr.id === ".", token);
        ...
        case Token.NumericLiteral:
          ...
          this.triggerAsync("warning", {
            ...
            return token.base === 16 && stateBinding.jsonMode;
          });
          ...
          this.triggerAsync("warning", {
            ...
            return stateBinding.isStrict() && token.base === 8 && token.isLegacy;
          });
